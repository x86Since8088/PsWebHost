...
Import-Module "$global:Project_Modules\Set-MissingContentItems_WS"
...
function New-Jwt {
    ...
    if ($null -eq $rsa) { # Requiring the private key to be present; else cannot sign!
        ...
        #[Security.Cryptography.sha]$SignatureAlgorithm = ('SHA' + ($rsa.SignatureAlgorithm -split '-' |?{$_ -match 'sha'} | %{[int]$_.replace('sha','')} | sort -Descending | select -First 1))
...
function Rebuild-JwtCertificate_WIP {
    [CmdletBinding()]
    param (
    ...
    if ($null -eq $rsa) { # Requiring the private key to be present; else cannot sign!
        ...
        $publiccspparams = [System.Security.Cryptography.CspParameters]::new($enhCsp.ProviderType, $enhCsp.ProviderName, $cert.PublicKey.key.CspKeyContainerInfo.KeyContainerName);
        $publicKey = [System.Security.Cryptography.RSACryptoServiceProvider]::new($publiccspparams);
...
#>
    ...
    # Force use of the Enhanced RSA and AES Cryptographic Provider with openssl-generated SHA256 keys
    $enhCsp = [System.Security.Cryptography.RSACryptoServiceProvider]::new().CspKeyContainerInfo;
    if ($cert.PublicKey.Key.CspKeyContainerInfo.ProviderType -eq 24) {
        $Publickey = $cert.PublicKey.key
    }
    elseif ($null -ne $cert.PublicKey.key.CspKeyContainerInfo.KeyContainerName) {
        $cspparams = [System.Security.Cryptography.CspParameters]::new($enhCsp.ProviderType, $enhCsp.ProviderName, $cert.PublicKey.key.CspKeyContainerInfo.KeyContainerName);
        $Publickey = [System.Security.Cryptography.RSACryptoServiceProvider]::new($cspparams);
    }
    else {
        write-error -Message "Unhandled Public key scenario during test-jwt."
    }
...
function Generate-JWT {
    param (
        ...
        $SecretKey = $Global:JWT_HS_secret
    )
...
Function New-JWTClaim {
    param (
        [string]$Issuer = (($url | Where-Object{$_ -match '^https.*?\w\.\w'} | Sort-Object length -Descending | select -First 1) + 'api/auth') ,
        [string]$Subject,
        [string]$Name,
        [string[]]$GroupNames,
        [string]$Audience,
    ...
    $GroupNames | Where-Object{$_} | Sort-Object -Unique | ForEach-Object{$HT.Add($_,$true)}
    $HT
}
...
Function Get-JWTCertificate {
    ...
    Select-Object *,@{Name='Length';Expression={$_.Name.Length}}|
    Sort-Object length -Descending | 
    Select-Object -First 1 ($Global:CertBindings[0].psobject.Properties.name|
...
Function Verify-JWT {
    ...
    return (Write-Warning "Verify-JWT is not complete.  Aborting.")
    #https://gist.github.com/SP3269/b852ddfe1319840ada2be0120a910132
    ...
    #$certx.Import($Certificate) # Usually available; reading from file here
    $Certx = $Certificate
    Function Add-Base64Padding ([string]$Text) {
        $NewText = $Text.replace('-','+').replace('_','/') # Decoding Base64url to the original byte array
        $mod = $NewText.Length % 4
        switch ($mod) {
            0 { $NewText = $NewText }
            1 { $NewText = $NewText.Substring(0,$NewTextToDecode.Length-1) }
            2 { $NewText = $NewText + "==" }
            3 { $NewText = $NewText + "=" }
        } # Done
    }
    $signed = Add-Base64Padding $parts[2]
