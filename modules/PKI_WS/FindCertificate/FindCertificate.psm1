    ...
    foreach ($URLItem in $URL) {
        if ($URLItem -match '^https') {
            ...
            if ($null -eq $CertMatch) {
                ...
                Out-Log_WS -Message 'Using substitute certificate where the FQDN does not match.' -Data ([pscustomobject]@{
                    URL=$URL;
                    Issuer=$CertMatch.Issuer
                    HasPrivateKey=$CertMatch.HasPrivateKey
                    Subject=$CertMatch.Subject
                    PublicCSP=$CertMatch.PublicKey.Key.CspKeyContainerInfo | select ProviderName,ProviderType
                    PrivateCSP=$CertMatch.PrivateKey.CspKeyContainerInfo | select ProviderName,ProviderType
                })
            }
